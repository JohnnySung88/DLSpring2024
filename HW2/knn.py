...
def compute_distances_two_loops(x_train, x_test):
  ...
  for train_ind in range(num_train):
      for test_ind in range(num_test):
          dists[train_ind,test_ind] = torch.square(x_train[train_ind]-x_test[test_ind]).sum()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  # Replace "pass" statement with your code
  for k in k_choices:
      k_to_accuracies[k] = [] # for the dictionary
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k,accs in sorted(k_to_accuracies.items()):
      avg.append(statistics.mean(accs))
  best_k = sorted(k_to_accuracies.items())[(torch.argmax(torch.tensor(avg))).item()][0]
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
