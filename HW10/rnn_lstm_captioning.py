...
class WordEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_size: int,device='cuda',dtype=torch.float32):
        super().__init__()
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        device='cuda'
        dtype=torch.float32
        self.featureExtractor = FeatureExtractor(pooling=(cell_type != 'attn'),
                                                  device=device, dtype=dtype)
...
class LSTM(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int,device='cuda',
                dtype=torch.float32):
        """
        ...
        # Register parameters
        self.Wx = nn.Parameter(
            torch.randn(input_dim, hidden_dim * 4,
                       device=device, dtype=dtype).div(math.sqrt(input_dim))
        )
        self.Wh = nn.Parameter(
            torch.randn(hidden_dim, hidden_dim * 4,
