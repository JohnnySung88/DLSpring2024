...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    probs[range(N),y] -= 1 # dL/dS
    dh = probs.mm(W2.t())  # dL/dH
    dz = probs.mm(W2.t())  # dL/dZ
    dz[z <= 0] = 0         # dL/dZ
    
    grads['W2'] = torch.mm(h.t(),probs)/N  # dL/dW2
    grads['b2'] = torch.sum(probs,dim=0)/N # dL/db2
    grads['W1'] = torch.mm(X.t(),dz)/N     # dL/dW1
    grads['b1'] = torch.sum(dz,dim=0)/N    # dL/db1
    grads['W1'] += 2*reg*W1
    grads['W2'] += 2*reg*W2
    ###########################################################################
    #                             END OF YOUR CODE                            #
