...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    scores = W.t().mv(X[i])
    scores = scores - scores.max() # For numeric stability
    correct_class_score = scores[y[i]]
    exp_scores = torch.exp(scores)
    exp_scores_total = torch.sum(exp_scores,axis = 0)
    loss += (-1)*torch.log(torch.exp(correct_class_score) / exp_scores_total)
    # Or can be written as "loss += -correct_class_score  + torch.log(torch.exp(scores).sum())"
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = np.linspace(1e-2, 2e-1, 5)
  regularization_strengths = np.linspace(1e-4, 1e-3, 5)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
