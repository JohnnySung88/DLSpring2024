...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    check=0
    x=torch.range(start,stop,dtype=torch.float64)
    mask=(x%10==0)
    x=x[mask]
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x[:2,1:6] = torch.tensor([[1,2,2,2,2], [1,2,2,2,2]])
    x[2:4,0:6] = torch.tensor([[3,4,3,4,5,5], [3,4,3,4,5,5]])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    #                      TODO: Implement this function                     #
    ##########################################################################
    index_col = torch.tensor([0,1,2])
    index_row = torch.tensor([1,0,3])
    y = x[index_row,index_col]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    min_ind_row = torch.argmin(x, dim = 1)
    y = torch.clone(x)
    y[range(x.shape[0]),min_ind_row] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
